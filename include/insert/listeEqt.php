<?
  ...
  if ($num_rows_om>0):
    while($dnom = $result_om->fetch(PDO::FETCH_ASSOC)):
      ...
      // Affichage de lobjet
      if ($_SESSION['mj']==1):
        $listeEqt.='  <div onClick="supprimerEqt('.$dnom['pom_id'].','.$p.')" class="icone lien"><i class="fa fa-trash" aria-hidden="true"></i></div>';
        $listeEqt.='  <div onClick="modifierEqt('.$dnom['pom_id'].','.$p.')" class="icone lien"><i class="fa fa-pencil" aria-hidden="true"></i></div>';
      endif;
