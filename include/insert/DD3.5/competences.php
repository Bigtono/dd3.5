...
echo '  <div class="carac_comp">Carac</div>';
echo '  <div class="formation_comp">Formation</div>';
echo '  <div class="armure_comp">Malus Armure</div>';
echo '</div>';    
  ...
  if ($num_rows > 0):
    while($dn = $result->fetch(PDO::FETCH_ASSOC)):
      ...
      echo '  <div id=formationComp'.$dn['comp_id'].'" class="formation_comp" onClick="'.$click.'">'.ouiNon($dn['comp_formationNecessaire']).'</div>';
      echo '  <div id="armureComp'.$dn['comp_id'].'" class="armure_comp" onClick="'.$click.'">'.ouiNon($dn['comp_malusArmure']).'</div>';
      echo '</div>';
    endwhile;
    else:
