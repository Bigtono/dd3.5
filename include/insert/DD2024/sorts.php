  ...
  if (strlen($critere_sql)>0):
    ...
    if ($_SESSION['critere_ls']>0):
      ...
      for($i=$min;$i<10;$i++):
        ...
        if (isset($_SESSION['critere_ls']) && $_SESSION['critere_ls'] !== "all"):
          ...
          $requete='SELECT DISTINCT so_id, so_nom, so_co_id, so_resume, so_res_id, res_selection, sc_niveau, res_abreviation FROM dd_sorts LEFT JOIN dd_ressources ON so_res_id=res_id LEFT JOIN dd_sortclasse ON so_id=sc_so_id WHERE so_res_id IN '.$selection.' AND sc_niveau='.$i.' ORDER BY so_nom';
        endif;
